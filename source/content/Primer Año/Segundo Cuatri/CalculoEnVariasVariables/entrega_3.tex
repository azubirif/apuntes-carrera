...
\item \textbf{[2 puntos]} $g(x,y)=(1+x^2+y^2)^{-1}$ en $(0,0)$.
		...
		\begin{itemize}
			...
			\item $\pdv{g}{x}{x}(x,y)= \frac{6x^{2}-2y^{2}-2}{(1+x^{2}+y^{2})^{3}}$
			\item $\pdv{g}{y}{y}(x,y)= \frac{6y^{2}-2x^{2}-2}{(1+x ^{2}+y ^{2})^{3}}$
			\item $\pdv{g}{x}{y}(x,y)= \frac{8xy}{(1+x ^{2}+y ^{2})^{3}}$
			\item $\pdv{g}{y}{x}(x,y)= \frac{8xy}{(1+x ^{2}+y ^{2})^{3}}$ 
		\end{itemize}
		Como vemos, $\grad{f}(0,0)=(0,0)$, por lo que solo debemos preocuparnos por la matriz Hessiana:
		\[
			H_{g}(0,0) = \begin{pmatrix} 
			-2 & 0 \\ 0 & -2
			\end{pmatrix} 
		\]
		Y ahora pasamos a calcular el producto:
		\[
			(\vb{x}-\vb{a})^{T}) H_{g}(\vb{a}) (\vb{x}-\vb{a})=
			\begin{pmatrix} x & y \end{pmatrix}
			\begin{pmatrix} -2 & 0 \\ 0 & -2 \end{pmatrix}
			\begin{pmatrix} 
			x \\ y	
			\end{pmatrix} =-2x^{2}-2y^{2}
		\]
		Y finalmente, el polinomio completo es
		\[
			P_{2,(0,0)} = 1-2(x^{2}+y^{2})
		\]
